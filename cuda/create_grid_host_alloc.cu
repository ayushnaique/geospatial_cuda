...
Grid* quadtree_grid(Point* d_grid_points, Point* d_points_array, int start, int count, int* h_grid_count, int* d_grid_count, pair<float, float> bottom_left_corner, pair<float, float> top_right_corner, int level) {
	...
	if (count < MIN_POINTS || (abs(x1 - x2) < MIN_DISTANCE and abs(y1 - y2) < MIN_DISTANCE)) {
        Point* subset = new Point[count];
        // Get only the subset of points that the current grid requires and return a grid
		if (count > 0) {
			cudaMemcpy(subset, d_points_array + start, sizeof(Point) * count, cudaMemcpyDeviceToHost);
		}
		return new Grid(nullptr, nullptr, nullptr, nullptr, subset, top_right_corner, bottom_left_corner, count);
	}
	...
	bl_grid = quadtree_grid(d_grid_points, d_points_array, start, g1, h_grid_count, d_grid_count, bottom_left_corner, mp(middle_x, middle_y), level + 1);
	br_grid = quadtree_grid(d_grid_points, d_points_array, br_start_pos, g2, h_grid_count, d_grid_count, mp(middle_x, y1), mp(x2, middle_y), level + 1);
	tl_grid = quadtree_grid(d_grid_points, d_points_array, tl_start_pos, g3, h_grid_count, d_grid_count, mp(x1, middle_y), mp(middle_x, y2), level + 1);
	tr_grid = quadtree_grid(d_grid_points, d_points_array, tr_start_pos, g4, h_grid_count, d_grid_count, mp(middle_x, middle_y), top_right_corner, level + 1);
    ...
    return new Grid(bl_grid, br_grid, tl_grid, tr_grid, subset, top_right_corner, bottom_left_corner, count);
}
...
int main(int argc, char *argv[]) {
	...
	cudaMemcpy(d_points_arr, points_array, point_count * sizeof(Point), cudaMemcpyHostToDevice);

	Grid* root_grid = quadtree_grid(d_grid_points, d_points_arr, 0, point_count, h_grid_count, d_grid_count, mp(0.0, 0.0), mp(max_size, max_size), 0);
