...
Grid *quadtree_grid(Point *points, Point *d_grid_points, int count,
	...
	if (count < MIN_POINTS or
		...
		return new Grid(nullptr, nullptr, nullptr, nullptr, points, top_right_corner,
						bottom_left_corner, count);
	}
	...
	vprint(
		"%d: Reorder in GPU: 1 block of %d threads each with range=%d\n",
    ...
    //     tr_start_pos = start_pos + h_grid_counts[0] + h_grid_counts[1] + h_grid_counts[2];

	// cudaMemcpy(bl, d_grid_points[start_pos], h_grid_counts[0] * sizeof(Point),
	// 		   cudaMemcpyDeviceToHost);
	// cudaMemcpy(br, d_grid_points[br_start_pos], h_grid_counts[1] * sizeof(Point),
	// 		   cudaMemcpyDeviceToHost);
	// cudaMemcpy(tl, d_grid_points[tl_start_pos], h_grid_counts[2] * sizeof(Point),
	// 		   cudaMemcpyDeviceToHost);
	// cudaMemcpy(tr, d_grid_points[tr_start_pos], h_grid_counts[3] * sizeof(Point),
	// 		   cudaMemcpyDeviceToHost);
	...
	// bl_grid = quadtree_grid(bl, d_grid_points, h_grid_counts[0], bottom_left_corner,
	// 						mp(middle_x, middle_y), start_pos, level + 1);
	// br_grid = quadtree_grid(br, d_grid_points, h_grid_counts[1], mp(middle_x, y1),
	// 						mp(x2, middle_y), br_start_pos, level + 1);
	// tl_grid = quadtree_grid(tl, d_grid_points, h_grid_counts[2], mp(x1, middle_y),
	// 						mp(middle_x, y2), tl_start_pos, level + 1);
	// tr_grid = quadtree_grid(tr, d_grid_points, h_grid_counts[3], mp(middle_x, middle_y),
	// 						top_right_corner, tr_start_pos, level + 1);

	// return new Grid(bl_grid, br_grid, tl_grid, tr_grid, points, top_right_corner,
	// 				bottom_left_corner, count);
    return new Grid(nullptr, nullptr, nullptr, nullptr, points, top_right_corner,
					bottom_left_corner, count);
}
...
int main(int argc, char *argv[]) {
	...
	Point *final_grid = (Point *)malloc(point_count * sizeof(Point));
	...
	start = clock();
	Grid *root_grid =
		quadtree_grid(points_array, d_grid_points, point_count, mp(0.0, 0.0),
					  mp(max_size, max_size), 0, 0);
	end = clock();

	cudaMemcpy(final_grid, d_grid_points, count * sizeof(Point),
			   cudaMemcpyDeviceToHost);
